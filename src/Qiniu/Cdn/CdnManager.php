...
final class CdnManager
{
    ...
    public function __construct(Auth $auth)
    {
        ...
        $this->api_server = 'http://api.qiniu.com';
    }
    ...
    */
    public function listSslcert($marker = null, $limit = 100)
    {
        $query = array('marker' => $marker);
        \Qiniu\setWithoutEmpty($query, 'limit', $limit);
        $url = $this->api_server . '/sslcert?' . http_build_query($query);
        return $this->get($url);
    }
    /*
    用户获取证书列表的接口
    ...
    */
    public function addSslcert($name,$common_name,$pri,$ca)
    {
      $req = array();
      $req['name'] = $name;
     ...
     * 用户删除证书的接口
     ...
     https://developer.qiniu.com/fusion/api/4248/certificate
     */
    public function deleteSslcert($certId)
    {
        $url = $this->api_server . '/sslcert/' . $certId;
    ...
    */
    ...
    public function ChangeDomainSslcert($domainName,$certid,$forceHttps=false)
    {
      $req = array();
      $req['certid'] = $certid;
      $req['forceHttps'] = $forceHttps;

      $url = $this->api_server . '/domain/' . $domainName .'/httpsconf' ;
      $body = json_encode($req);
